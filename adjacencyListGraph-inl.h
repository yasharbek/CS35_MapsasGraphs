...
void AdjacencyListGraph<V, E, W>::removeVertex(V v) {
  if (! this->vertices.contains(v)) {
    throw runtime_error(std::string("Vertex ") + cs35::to_string(v) +
                            " not present in graph::removeVertex");
  }
  if (getOutgoingEdges(v).size() > 0 ||
      getIncomingEdges(v).size() > 0) {
    throw runtime_error(std::string("Vertex ") + cs35::to_string(v) +
                            " still has edges::removeVertex");
  }
...
void AdjacencyListGraph<V, E, W>::insertEdge(V src, V dest, E label, W weight) {
    if (!containsVertex(src)) {
        throw runtime_error(std::string("Vertex ") + cs35::to_string(src) +
                            " not present in graph::insertEdge");
    }
